...
std::string navIcon(const std::string& name) {
    ...
    if (name == "companies") return "<svg viewBox='0 0 24 24' aria-hidden='true'><path d='M3 21h18'></path><path d='M5 21V7l7-4 7 4v14'></path><path d='M9 10h1'></path><path d='M14 10h1'></path><path d='M9 14h1'></path><path d='M14 14h1'></path></svg>";
    ...
    if (name == "ratings") return "<svg viewBox='0 0 24 24' aria-hidden='true'><path d='m12 3 2.7 5.5 6.1.9-4.4 4.3 1 6.1-5.4-2.9-5.4 2.9 1-6.1-4.4-4.3 6.1-.9L12 3z'></path></svg>";
...
std::string mobileTabBar(const std::string& active, const std::string& user_email) {
    ...
    out << "<nav class='mobile-tabbar' aria-label='Основная навигация'>"
        ...
        << "<a class='" << activeClass(active, "companies") << "' href='/companies'>" << navIcon("companies") << "<span>Компании</span></a>"
        << "<a class='" << activeClass(active, "applications") << "' href='/applications'>" << navIcon("applications") << "<span>Подачи</span></a>"
        << "<a class='" << activeClass(active, "ratings") << "' href='/ratings'>" << navIcon("ratings") << "<span>Оценки</span></a>"
...
Response ApplicationController::board() const {
    ...
    for (ApplicationStatus status : statuses) {
        ...
        b << "<details class='kanban-column' data-status='" << status_text << "' open><summary class='kanban-column-head'><h2>" << label(status) << "</h2><span class='kanban-count'>" << count << "</span></summary><div class='kanban-list' data-status='" << status_text << "'>";
        for (const auto& row : rows) {
    ...
    b << "</section>";
    b << "<script>(()=>{const board=document.getElementById('kanbanBoard');const archiveDropzone=document.getElementById('archiveDropzone');if(!board)return;let draggedCard=null;const mobile=()=>matchMedia('(max-width: 900px)').matches;const updateCounter=(column)=>{const count=column.querySelectorAll('.kanban-card').length;const node=column.querySelector('.kanban-count');if(node)node.textContent=String(count);};const syncStatus=async(appId,status)=>{const response=await fetch(`/applications/${appId}/move`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({status})});if(!response.ok)throw new Error('Не удалось обновить статус.');};const moveToArchive=async(appId)=>{const response=await fetch(`/applications/${appId}/archive-move`,{method:'POST',headers:{'Content-Type':'application/json'}});return response.ok;};const collapseMobile=()=>{board.querySelectorAll('.kanban-column').forEach((column)=>{if(mobile())column.removeAttribute('open');else column.setAttribute('open','');});};collapseMobile();addEventListener('resize',collapseMobile);const finishMove=async(column)=>{if(!draggedCard)return;const list=column.querySelector('.kanban-list');const fromColumn=draggedCard.closest('.kanban-column');const newStatus=column.dataset.status;const appId=draggedCard.dataset.appId;list.appendChild(draggedCard);updateCounter(fromColumn);updateCounter(column);if(mobile())column.removeAttribute('open');column.scrollIntoView({behavior:'smooth',block:'center'});try{await syncStatus(appId,newStatus);}catch(error){fromColumn.querySelector('.kanban-list').appendChild(draggedCard);updateCounter(fromColumn);updateCounter(column);alert(error.message);}};const dragOver=(event)=>{event.preventDefault();const y=event.clientY;if(y>innerHeight-120)scrollBy({top:12,behavior:'smooth'});if(y<90)scrollBy({top:-12,behavior:'smooth'});};board.querySelectorAll('.kanban-card').forEach((card)=>{card.addEventListener('dragstart',()=>{draggedCard=card;card.classList.add('dragging');});card.addEventListener('dragend',()=>{card.classList.remove('dragging');draggedCard=null;board.querySelectorAll('.drag-over').forEach((node)=>node.classList.remove('drag-over'));});});board.querySelectorAll('.kanban-list,.kanban-column-head').forEach((target)=>{target.addEventListener('dragover',(event)=>{dragOver(event);target.classList.add('drag-over');});target.addEventListener('dragleave',()=>target.classList.remove('drag-over'));target.addEventListener('drop',async(event)=>{event.preventDefault();event.stopPropagation();target.classList.remove('drag-over');await finishMove(target.closest('.kanban-column'));});});if(archiveDropzone){archiveDropzone.addEventListener('dragover',(event)=>{dragOver(event);archiveDropzone.classList.add('drag-over');});archiveDropzone.addEventListener('dragleave',()=>archiveDropzone.classList.remove('drag-over'));archiveDropzone.addEventListener('drop',async(event)=>{event.preventDefault();archiveDropzone.classList.remove('drag-over');if(!draggedCard)return;const fromColumn=draggedCard.closest('.kanban-column');const appId=draggedCard.dataset.appId;if(await moveToArchive(appId)){draggedCard.remove();updateCounter(fromColumn);archiveDropzone.scrollIntoView({behavior:'smooth',block:'center'});}else alert('Не удалось перенести в архив.');});}})();</script>";
    Response response;
    UserRepository users(db_);
    Row user = users.findById(user_id_);
